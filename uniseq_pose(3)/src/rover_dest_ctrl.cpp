...
struct Square2D{
    ...
    Point2D bot_r;
    double len_vrt(){
        return abs(top_l.x - bot_l.x);
    }
    double len_hor(){
        return abs(bot_l.y - bot_r.y);
    }
};
...
class Grid{
    public:
        ...
        void fin_grid( double x, double y ){
            fin.top_l.x = find_range( x, brd.bot_l.x, brd.top_l.x, step_vrt );
            fin.bot_l.x = fin.top_l.x - step_vrt;

            fin.bot_r.y = find_range( y, brd.bot_l.y, brd.bot_r.y, step_hor );
            fin.bot_l.y = fin.bot_r.y - step_hor;

            fin.top_l.y = fin.bot_l.y;
            fin.bot_r.x = fin.bot_l.x;
            fin.top_r.x = fin.top_l.x;
            fin.top_r.y = fin.bot_r.y;
        }
