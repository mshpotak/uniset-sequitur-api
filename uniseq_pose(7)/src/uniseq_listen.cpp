...
std::ofstream data_ofs("data_out.csv", std::ofstream::trunc);
...
void dataCallback( const uniseq_pose::SequiturData::ConstPtr& msg ){
    ROS_INFO( "[count = %d:\tx = %f\ty = %f\tz = %f]",
    msg->header.seq,
    msg->pose.position.x,
    msg->pose.position.y,
...
int main( int argc, char *argv[] ){
    ...
    ros::Subscriber data_sub = hand.subscribe( "sequitur_data", 1000, dataCallback );
    ros::spin();
    return 0;
}
